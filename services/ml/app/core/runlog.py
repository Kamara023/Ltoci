"""Journalisation des runs du service ML dans ops.ingestion_runs
(même pattern que le service ingestion — source dédiée 'stats-engine')."""
...
class RunLogger:
    def __init__(self, source_code: str, triggered_by: str):
        ...
        with get_engine().begin() as conn:
            ...
            self.run_id = conn.execute(
                insert(runs)
                .values(source_id=source_id, triggered_by=triggered_by, status="RUNNING", stats={})
                .returning(runs.c.id)
            ).scalar_one()
